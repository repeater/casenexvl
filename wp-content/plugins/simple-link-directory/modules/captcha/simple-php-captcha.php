...
//
function sld_simple_php_captcha($config = array()) {
    ...
    $bg_path = QCOPD_DIR_MOD . '/captcha/backgrounds/';
    $font_path = QCOPD_DIR_MOD . '/captcha/fonts/';
...
if( !function_exists('sld_hex2rgb') ) {
    function sld_hex2rgb($hex_str, $return_string = false, $separator = ',') {
...
if( isset($_GET['_CAPTCHA']) ) {
    ...
    $color = sld_hex2rgb($captcha_config['color']);
    ...
    if( $captcha_config['shadow'] ){
        $shadow_color = sld_hex2rgb($captcha_config['shadow_color']);
