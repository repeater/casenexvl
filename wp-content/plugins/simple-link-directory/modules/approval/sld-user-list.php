...
class qc_sld_user_list {

	/**
	 * The only instance of qc_sld_user_list.
	 *
	 * @var qc_sld_user_list
	 */
	 ...
	 *
	 * @return qc_sld_user_list
	 */
	public static function instance() {
		if ( !isset( self::$instance ) ) {
			self::$instance = new qc_sld_user_list();
		}
	...
	private function __construct() {
		...
		add_action('show_user_profile', array($this,'qc_custom_user_profile_fields'));
		add_action('edit_user_profile', array($this,'qc_custom_user_profile_fields'));
		
		add_action( 'personal_options_update', array($this,'qc_save_extra_user_profile_fields') );
		
		add_action( 'edit_user_profile_update', array($this,'qc_save_extra_user_profile_fields') );
	...
	public function update_action() {
		if ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'approve', 'deny' ) ) && !isset( $_GET['new_role'] ) ) {
			...
			qc_sld_user_approval()->update_user_status( $user, $status );
	...
	public function user_table_actions( $actions, $user ) {
		...
		$user_status = qc_sld_user_approval()->get_user_status( $user->ID );

		$approve_link = add_query_arg( array( 'action' => 'approve', 'user' => $user->ID ) );
		$approve_link = remove_query_arg( array( 'new_role' ), $approve_link );
		$approve_link = wp_nonce_url( $approve_link, 'sld' );

		$deny_link = add_query_arg( array( 'action' => 'deny', 'user' => $user->ID ) );
		$deny_link = remove_query_arg( array( 'new_role' ), $deny_link );
		$deny_link = wp_nonce_url( $deny_link, 'sld' );

		$approve_action = '<a href="' . esc_url( $approve_link ) . '">' . __( 'Approve', 'sld' ) . '</a>';
		$deny_action = '<a href="' . esc_url( $deny_link ) . '">' . __( 'Deny', 'sld' ) . '</a>';
	...
	public function add_column( $columns ) {
		$the_columns['sld_user_status'] = __( 'Status', 'sld' );
	...
	public function status_column( $val, $column_name, $user_id ) {
		switch ( $column_name ) {
			case 'sld_user_status' :
				$status = qc_sld_user_approval()->get_user_status( $user_id );
				if ( $status == 'approved' ) {
					$status_i18n = __( 'approved', 'sld' );
				} else if ( $status == 'denied' ) {
					$status_i18n = __( 'denied', 'sld' );
				} else if ( $status == 'pending' ) {
					$status_i18n = __( 'pending', 'sld' );
				}
				return $status_i18n;
				break;
	...
	public function status_filter( $which ) {
		$id = 'qc_sld_filter-' . $which;

		$filter_button = submit_button( __( 'Filter', 'sld' ), 'button', 'pw-status-query-submit', false, array( 'id' => 'pw-status-query-submit' ) );
		$filtered_status = $this->selected_status();

		?>
		<label class="screen-reader-text" for="<?php echo $id ?>"><?php _e( 'View all users', 'sld' ); ?></label>
		<select id="<?php echo $id ?>" name="<?php echo $id ?>" style="float: none; margin: 0 0 0 15px;">
			<option value=""><?php _e( 'View all users', 'sld' ); ?></option>
		<?php foreach ( qc_sld_user_approval()->get_user_statuses() as $status => $users ) : ?>
			<?php if ( count( $users ) ) : ?>
			<option value="<?php echo esc_attr( $status ); ?>"<?php selected( $status, $filtered_status ); ?>><?php echo esc_html( $status ); ?></option>
			<?php endif; ?>
		<?php endforeach; ?>
		</select>
		<?php echo apply_filters( 'qc_sld_filter_button', $filter_button ); ?>
		<style>
	...
	public function filter_by_status( $query ) {
		...
		if ( $this->selected_status() != null ) {
			...
			if ( 'approved' == $filter ) {
				...
				$query->query_from .= " LEFT JOIN {$wpdb->usermeta} AS mt1 ON ({$wpdb->users}.ID = mt1.user_id AND mt1.meta_key = 'sld_user_status')";
				$query->query_where .= " AND ( ( wp_usermeta.meta_key = 'sld_user_status' AND CAST(wp_usermeta.meta_value AS CHAR) = 'approved' ) OR mt1.user_id IS NULL )";
			} else {
				$query->query_where .= " AND ( (wp_usermeta.meta_key = 'sld_user_status' AND CAST(wp_usermeta.meta_value AS CHAR) = '{$filter}') )";
			}
		}
	}

	private function selected_status() {
		if ( ! empty( $_REQUEST['qc_sld_filter-top'] ) || ! empty( $_REQUEST['qc_sld_filter-bottom'] ) ) {
			return esc_attr( ( ! empty( $_REQUEST['qc_sld_filter-top'] ) ) ? $_REQUEST['qc_sld_filter-top'] : $_REQUEST['qc_sld_filter-bottom'] );
		}
		...
		if ( $screen->id == 'users' ) : ?>
			<script type="text/javascript">
				jQuery(document).ready(function ($) {
					$('<option>').val('approve').text('<?php _e( 'Approve', 'sld' )?>').appendTo("select[name='action']");
					$('<option>').val('approve').text('<?php _e( 'Approve', 'sld' )?>').appendTo("select[name='action2']");

					$('<option>').val('deny').text('<?php _e( 'Deny', 'sld' )?>').appendTo("select[name='action']");
					$('<option>').val('deny').text('<?php _e( 'Deny', 'sld' )?>').appendTo("select[name='action2']");
				});
			</script>
		<?php endif;
	}
	...
	public function bulk_action() {
		...
		if ( $screen->id == 'users' ) {
			...
			$sendback = remove_query_arg( array( 'approved', 'denied', 'deleted', 'ids', 'qc_sld_filter', 'qc_sld_filter2', 'pw-status-query-submit', 'new_role' ), wp_get_referer() );
			if ( !$sendback ) {
				$sendback = admin_url( 'users.php' );
			}
			...
			switch ( $action ) {
				case 'approve':
					...
					foreach ( $user_ids as $user_id ) {
						qc_sld_user_approval()->update_user_status( $user_id, 'approve' );
						$approved++;
					}
				...
				case 'deny':
					$denied = 0;
					foreach ( $user_ids as $user_id ) {
						qc_sld_user_approval()->update_user_status( $user_id, 'deny' );
						$denied++;
					}
	...
	public function admin_notices() {
		...
		if ( isset( $_REQUEST['denied'] ) && (int) $_REQUEST['denied'] ) {
			...
			$message = sprintf( _n( 'User denied.', '%s users denied.', $denied, 'sld' ), number_format_i18n( $denied ) );
		}

		if ( isset( $_REQUEST['approved'] ) && (int) $_REQUEST['approved'] ) {
			$approved = esc_attr( $_REQUEST['approved'] );
			$message = sprintf( _n( 'User approved.', '%s users approved.', $approved, 'sld' ), number_format_i18n( $approved ) );
		}
	...
	public function profile_status_field( $user ) {
		...
		<table class="form-table">
			<tr>
				<th><label for="qc_sld_status"><?php _e( 'Access Status', 'sld' ); ?></label>
				</th>
				<td>
					<select id="qc_sld_status" name="qc_sld_status">
						<?php if ( $user_status == 'pending' ) : ?>
							<option value=""><?php _e( '-- Status --', 'sld' ); ?></option>
						<?php endif; ?>
					...
					<span
						class="description"><?php _e( 'If user has access to sign in or not.', 'sld' ); ?></span>
					<?php if ( $user_status == 'pending' ) : ?>
						<br/><span
							class="description"><?php _e( 'Current user status is <strong>pending</strong>.', 'sld' ); ?></span>
					<?php endif; ?>
				</td>
			</tr>
		</table>
...
/*
*
* This is just a Testing function only
*
*/
public function qc_custom_user_profile_fields( $user ){
	$occutpation = get_user_meta($user->ID,'qc_sld_user_occupation',true);
?>
		<table class="form-table">
			<tr>
				<th><label for="qc_sld_user_occupation"><?php _e( 'Occupation', 'sld' ); ?></label>
				</th>
				<td>
					<input type="text" name="qc_sld_user_occupation" id="qc_sld_user_occupation" value="<?php echo $occutpation; ?>" />
					<span
						class="description"><?php _e( 'If user has Occupation.', 'sld' ); ?></span>
...
public function qc_save_extra_user_profile_fields( $user ){
	if(!empty($_POST['qc_sld_user_occupation'])){
		update_user_meta($user,'qc_sld_user_occupation',esc_attr($_POST['qc_sld_user_occupation']));
	}
}
	...
	public function save_profile_status_field( $user_id ) {
		...
		if ( !empty( $_POST['qc_sld_status'] ) ) {
			$new_status = esc_attr( $_POST['qc_sld_status'] );

			if ( $new_status == 'approved' )
				$new_status = 'approve'; else if ( $new_status == 'denied' )
				$new_status = 'deny';

			qc_sld_user_approval()->update_user_status( $user_id, $new_status );
		}
	...
	public function pending_users_bubble() {
		...
		$users = qc_sld_user_approval()->get_user_statuses();
...
function qc_sld_user_list() {
	return qc_sld_user_list::instance();
}
