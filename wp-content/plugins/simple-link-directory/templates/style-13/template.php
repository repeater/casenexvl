...
<?php wp_enqueue_style('sld-css-style-13', OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
<style>
		...
		<div id="bookmark_list" class="qc-grid-item qcopd-list-column opd-column-<?php echo 1; echo " " . $style;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					?>
					<li id="sld_bookmark_li_<?php echo $b; ?>" class="ilist-25">
								...
								<div class="ilist-item-main">
									...
									<?php endif; ?>
										...
										<div class="item-title-text">
												<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo (isset($list['qcopd_item_nofollow']) && $list['qcopd_item_nofollow'] == 1) ? 'rel="nofollow"' : ''; ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> <?php echo (isset($outbound_conf) && $outbound_conf == 1) ? 'onclick="trackOutboundLink(\''.$item_url.'\', this); return false;"' : ''; ?> ><h3 class="item-list-title"><?php
													echo trim($list['qcopd_item_title']);
												?></h3></a>

												<div class="bookmark-section bookmark-section-style-13">
...
<?php
	...
	while ( $list_query->have_posts() )
	{
		...
		<style>
			...
			#qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?> ul li .item-title-text h3{
			  color: <?php echo $conf['list_txt_color']; ?>;
				<?php if($conf['item_title_font_size']!=''): ?>
				font-size:<?php echo $conf['item_title_font_size']; ?> !important;
				<?php endif; ?>
			...
			#qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?> ul li .item-title-text h3:hover{
			  color: <?php echo $conf['list_txt_color_hov']; ?>;
			}

			#qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?> ul li .item-desc-text p{
			  color: <?php echo $conf['list_subtxt_color']; ?>;
				<?php if($conf['item_subtitle_font_size']!=''): ?>
				font-size:<?php echo $conf['item_subtitle_font_size']; ?> !important;
				<?php endif; ?>
			...
			#qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?> ul li .item-desc-text:hover p{
			  color: <?php echo $conf['list_subtxt_color_hov']; ?>;
			}
		...
		<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-grid-item qcopd-list-column opd-column-<?php echo 1; echo " " . $style;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">

			<div class="opd-list-style-12 sld-container title_style_sld qcopd-single-list" style="<?php echo ($listId==1)?'margin-top:10px':''; ?>">
				...
				<ul class="tooltip_tpl12-tpl sld-list" id="jp-list-<?php echo get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
					...
					?>
					<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="ilist-25">
						...
						<?php endif; ?>
							...
							<?php endif; ?>
												...
												<?php if( $upvote == 'on' ) : ?>
													...
													<!-- upvote section -->
													<div class="upvote-section upvote-section-style-13 upvote">
