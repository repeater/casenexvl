...
<?php wp_enqueue_style('sld-css-style-11-tooltip', OCOPD_TPL_URL . "/$template_code/css/darktooltip.css" ); ?>
<?php wp_enqueue_style('sld-css-style-11', OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
<?php wp_enqueue_script('sld-css-style-11-tooltip-js', OCOPD_TPL_URL . "/$template_code/css/jquery.darktooltip.js"); ?>
...
?>
		...
		<style>
			#bookmark_list.style-1 .ca-menu li {
			    border-left: 10px solid <?php echo $conf['list_border_color']; ?>;
			    background-color: <?php echo $conf['list_bg_color']; ?>;
				box-shadow: 1px 1px 2px <?php echo $conf['item_bdr_color']; ?>;
			}

			#bookmark_list.style-1 .ca-menu li:hover {
			...
			#bookmark_list.style-1 .ca-menu li .ca-main {
			...
			#bookmark_list.style-1 .ca-menu li:hover .ca-main {
			  color: <?php echo $conf['list_txt_color_hov']; ?>;
			}

			#bookmark_list.style-1 .ca-menu li .ca-sub {
			...
			#bookmark_list.style-1 .ca-menu li:hover .ca-sub {
			  color: <?php echo $conf['list_subtxt_color_hov']; ?>;
			}

			#bookmark_list.style-1 .ca-menu li .upvote-section .upvote-btn, #qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?> .ca-menu li .upvote-section .upvote-count{
			  color: <?php echo $conf['list_subtxt_color']; ?>;
			}

			#bookmark_list.style-1 .ca-menu li:hover .upvote-section .upvote-btn, #bookmark_list.style-1 .ca-menu li:hover .upvote-section .upvote-count{
			  color: <?php echo $conf['list_subtxt_color_hov']; ?>;
			}
		...
		<div id="bookmark_list" class="qc-grid-item qcopd-list-column <?php echo $style; ?> <?php echo "opd-list-id-" . get_the_ID(); ?>">

			<div class="qcopd-single-list style-12-div">
				<h2><?php echo ($b_title!=''?$b_title:'Quick Links'); ?></h2>
				<div>
			       <ul class="tpl-12-img-logo-list" id="sld_bookmark_ul">
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					<?php
					$sub = "";
					if( isset($list['qcopd_item_subtitle']) ) {
						$sub = trim($list['qcopd_item_subtitle']);
					}
					?>

					<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="tooltip_tpl11 sld-style11-column<?php echo $column; ?>" data-tooltip="<?php echo $sub; ?>">
						<?php
						...
						?>
						<!-- List Anchor -->
						<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo (isset($list['qcopd_item_nofollow']) && $list['qcopd_item_nofollow'] == 1) ? 'rel="nofollow"' : ''; ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-title="<?php echo $sub; ?>" >
							...
							<?php if( ($list_img == "true") && isset($list['qcopd_item_img'])  && $list['qcopd_item_img'] != "" ) : ?>
								...
								?>
								<img src="<?php echo $img[0]; ?>" alt="<?php echo trim($list['qcopd_item_title']); ?>">
							<?php elseif( $iconClass != "" ) : ?>
								<span class="icon">
									<i class="fa <?php echo $iconClass; ?>"></i>
								</span>
							<?php elseif( $showFavicon == 1 && $faviconFetchable == true ) : ?>
								<span class="icon logo-icon favicon-loaded">
									<img src="<?php echo $faviconImgUrl; ?>" alt="<?php echo $list['qcopd_item_title']; ?>">
								</span>
							<?php else : ?>
								<img src="<?php echo QCOPD_IMG_URL; ?>/list-image-placeholder.png" alt="<?php echo trim($list['qcopd_item_title']); ?>">
							<?php endif; ?>

                        </a>
						<div class="style-11-upvote-section">
                        <div class="bookmark-section bookmark-section-style-11">
...
//adding extra variable in config
		...
		<style>
			#item-<?php echo $listId .'-'. get_the_ID(); ?>-add-block {
			  margin-top: 35px;
			  border-radius: 5px;
		...
		<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-grid-item qcopd-list-column <?php echo $style; ?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
			...
			<div class="qcopd-single-list style-12-div">
				...
				<div>
			       <ul class="tpl-12-img-logo-list" id="jp-list-<?php echo get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
					<?php $count = 1; 
					if( $item_orderby == 'upvotes' )
						{
    						usort($lists, "custom_sort_by_tpl_upvotes");
					...
					<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="tooltip_tpl11 sld-style11-column<?php echo $column; ?>" data-tooltip="<?php echo $sub; ?>">
                        ...
                        <?php if( $upvote == 'on' ) : ?>
							...
							<!-- upvote section -->
							<div class="upvote-section upvote-section-style-11">
