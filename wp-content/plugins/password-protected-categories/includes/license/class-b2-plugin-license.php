...
if ( ! class_exists( 'Barn2_Plugin_License' ) ) {
	...
	class Barn2_Plugin_License {
						...
						switch ( $license_data->error ) {
							case 'expired' :
								$error_message	 = sprintf(
									__( 'Your license key expired on %s.', 'password-protected-categories' ), date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
								);
								break;
							case 'revoked' :
								$error_message	 = __( 'Your license key has been disabled.', 'password-protected-categories' );
								break;
							case 'missing' :
							case 'item_name_mismatch' :
								$error_message	 = __( 'Your license key is invalid.', 'password-protected-categories' );
								break;
							case 'invalid' :
							case 'site_inactive' :
								$error_message	 = __( 'Your license is not active for this URL.', 'password-protected-categories' );
								break;
							case 'no_activations_left':
								$error_message	 = __( 'Your license key has reached its activation limit.', 'password-protected-categories' );
								break;
							default :
								$error_message	 = __( 'There was an error activating your license, please try again.', 'password-protected-categories' );
								$result			 = 'error';
								break;
						}
					} // if license invalid
		...
		public function get_license_key_admin_message() {
			$message			 = __( 'Please enter your license key.', 'password-protected-categories' );
			$deactivate_button	 = sprintf( '<button type="submit" class="button" name="deactivate_key" value="%1$s" style="margin:3px 0 0 8px;">%2$s</button>', esc_attr( $this->license_key_option ), __( 'Deactivate', 'password-protected-categories' ) );

			if ( $this->is_valid() ) {
				$message = sprintf( '<span style="color:green;">✓ %s</span>', __( 'License key successfully activated.', 'password-protected-categories' ) ) . $deactivate_button;
			} elseif ( $this->is_deactivated() ) {
				$message = __( 'License key successfully deactivated. Click Save to reactivate.', 'password-protected-categories' );
			} elseif ( $this->failed_deactivation() ) {
				$message = sprintf( '<span style="color:red;">%s</span>', __( 'There was an error deactivating your license key, please try again.', 'password-protected-categories' ) ) . $deactivate_button;
			} elseif ( $this->is_invalid() ) {
				$message = $this->get_license_error();
				if ( ! $message ) {
					$message = __( 'There was an error activating your license, please try again.', 'password-protected-categories' );
				}
		...
		private function get_api_error_message( $response ) {
			if ( is_wp_error( $response ) ) {
				...
				return __( 'An error has occurred, please try again.', 'password-protected-categories' );
			}
		}
