		...
		<tbody>
			<tr valign="top">
				...
				<td>
					...
					<label for="enabled">
                    <input type="checkbox" name="enabled" value="1" <?php if ($config->enabled) { echo 'checked="1"'; } if ($config->forcedEnabled || $config->notReady) { ?> readonly="1" <?php } ?>/>  CDN Functionality Enabled
                    <?php if ($config->forcedEnabled) { ?> <br /><b>Config set by define(), see wp-config.php</b> <?php } ?>
                    <?php if ($config->notReady) { ?> <br /><b>Auto disabled until the CDN is fully deployed (Up to 20 minutes).</b> <?php } ?>
					</label>
				</td>
			</tr>
			<tr valign="top">
				...
				<td>
                <input type="text" name="cdn_url" value="<?php echo esc_html($config->cdn_url); ?>" size="80" class="regular-text code" id="cdn_url" <?php if ($config->forcedCdnUrl) { ?> readonly="1" <?php } ?>/>
                    ...
                    <?php if ($config->forcedCdnUrl) { ?> <br /><b>Config set by define(), see wp-config.php</b> <?php } ?>
					</p>
				</td>
			</tr>
			<tr valign="top">
				...
				<td>
                <input type="text" name="https_cdn_url" value="<?php echo  esc_html($config->https_cdn_url); ?>" size="80" class="regular-text code" id="https_cdn_url" <?php if ($config->forcedHttpsUrl) { ?> readonly="1" <?php } ?> />
                    ...
                    <?php if ($config->forcedHttpsUrl) { ?> <br /><b>Config set by define(), see wp-config.php</b> <?php } ?>
					</p>
				</td>
			</tr>
			<tr valign="top">
